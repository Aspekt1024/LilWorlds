    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                o.uv = v.uv; //TRANSFORM_TEX(v.uv, _WaterTex);
                float d = saturate(dot(float3(0,1,0), v.normal));
                
                float w = saturate((_WaterHeight - v.vertex.y) * (1 / _WaterHeight) * 2);
                float g = 0;
                float m = 0;

                float dp = pow(d, 100);
                float p = 1 - saturate((_SnowHeight - v.vertex.y) * (1 / _SnowHeight) * 5);
                p = saturate(p * (pow(d, 6)) * 100);
                g = saturate((_GrassMaxHeight - v.vertex.y) * (1 / _GrassMaxHeight) * 5);
                g = saturate((g - w - p) * (dp) * 10000);
