    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float d = saturate(dot(normalize(v.vertex), v.normal));
                float dp = pow(d, 100);

                float h = length(v.vertex.xyz);
                float floorLevel = _MinHeight + _FloorLevel * (_MaxHeight - _MinHeight);

                half w = v.vertex.y < _FloorLevel + 0.04 ? 1 : 0;
                half p = v.vertex.y > _Size * 0.2 ? saturate((d - 0.5) * 10) : 0;
                half g = v.vertex.y > _FloorLevel + 0.02 ? saturate((d - 0.5) * 10) - w - p : 0;
                half m = 1 - g - w - p;
                w = h < floorLevel ? 1 : 0;
                p = h > _MinHeight + (_MaxHeight - _MinHeight) * 0.85 ? 1 : 0;
                g = h > floorLevel ? saturate((d - 0.5) * 10 - w - p) : 0;
                m = 1 - g - w - p;
                ...
                o.col.rgba *= shadow * _LightColor0;
                TRANSFER_SHADOW(o)
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                col.rgb += triplanar(i.wpos, i.normal, _GrassTex, 2) * i.col.g;
